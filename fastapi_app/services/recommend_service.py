...
class RecommendService:
    ...
    def _get_food_type_keywords(self, preferred_types: List[str]) -> set:
        """
        사용자 선호 음식 종류를 content 필드 매칭용 키워드로 변환

        Args:
            preferred_types: 사용자 선호 음식 종류 리스트

        Returns:
            매칭용 키워드 set (소문자)
        """
        # 음식 종류 매핑 (사용자 입력 → content 필드 매칭)
        all_food_types = {
            "한식": ["한국음식", "한식"],
            "한국음식": ["한국음식", "한식"],
            "중식": ["중식", "중국음식"],
        ...
        # 사용자가 선호하는 음식 종류 키워드만 추출
        preferred_keywords = set()
        for pref in preferred_types:
            pref_lower = pref.lower()
            if pref_lower in all_food_types:
                preferred_keywords.update([k.lower() for k in all_food_types[pref_lower]])
            else:
                preferred_keywords.add(pref_lower)

        return preferred_keywords

    def _filter_by_food_type(self, user, category: str, candidates: List[dict]) -> List[dict]:
        """
        사용자 선호 음식 종류에 맞지 않는 후보를 필터링

        Args:
            user: 사용자 정보
            category: restaurant 또는 cafe
            candidates: 후보 POI 리스트
        ...
        if category == "restaurant" and hasattr(user, 'preferred_restaurant_types') and user.preferred_restaurant_types:
            preferred_types = [t.lower() for t in user.preferred_restaurant_types]
        elif category == "cafe" and hasattr(user, 'preferred_cafe_types') and user.preferred_cafe_types:
            preferred_types = [t.lower() for t in user.preferred_cafe_types]

        if not preferred_types:
            return candidates  # 선호도가 없으면 필터링 안함
        ...
        preferred_keywords = set()
        for pref in preferred_types:
            if pref in all_food_types:
                preferred_keywords.update([k.lower() for k in all_food_types[pref]])
            else:
                preferred_keywords.add(pref)

        # 필터링: 선호 키워드가 포함된 것만 허용
        filtered = []
        for poi in candidates:
            ...
            if not content:
                # content가 없으면 제외
                print(f"[RERANK] Filtered out: {meta.get('name', 'N/A')} (food_type: content 없음)")
                continue

            # 선호 음식 종류 키워드가 포함되어 있는지 확인
            has_preferred = any(keyword in content for keyword in preferred_keywords)

            if has_preferred:
                # 선호 음식 종류가 포함되어 있으면 포함
                filtered.append(poi)
            else:
                # 선호 음식 종류가 없으면 제외
                print(f"[RERANK] Filtered out: {meta.get('name', 'N/A')} (food_type: {content})")
    ...
    def _llm_rerank(self, user, category: str, candidates: List[dict], top_k: int = 10, debug: bool = False) -> List[dict]:
        ...
        # 음식 종류 필터링 (restaurant/cafe인 경우)
        if category in ["restaurant", "cafe"]:
            # 디버깅: 원본 후보의 음식 종류 분포 확인
            print(f"\n[RERANK] Before filtering - Total candidates: {len(candidates)}")
            for idx, poi in enumerate(candidates[:5]):  # 상위 5개만 출력
                meta = poi.get("_meta", {})
                print(f"  {idx+1}. {meta.get('name', 'N/A')}: {meta.get('content', 'N/A')}")

            filtered_candidates = self._filter_by_food_type(user, category, candidates)
            if len(filtered_candidates) > 0:
                # 필터링 결과가 있으면 사용 (개수가 적어도 선호 음식만 추천)
                candidates = filtered_candidates
                print(f"[RERANK] Food type filtering: {len(candidates)} candidates after filtering\n")
            else:
                # 필터링 결과가 0개면 원본 유지
                print(f"[RERANK] Food type filtering: No matches, keeping all {len(candidates)} candidates\n")
...
ranked_indices는 위 후보 목록의 index 값들을 재정렬한 배열입니다."""
    ...
    def recommend(self, user, top_k_per_category: int = 10, distance_max_km: float = 3.0, debug: bool = False) -> List[dict]:
        ...
        for scorer in self.scorers:
            ...
            # 임베딩 기반으로 후보 추출 (reranking을 위한 후보군)
            # 음식 종류 필터링이 필요한 경우 더 많이 추출
            has_food_preference = (
                (scorer.name == "restaurant" and hasattr(user, 'preferred_restaurant_types') and user.preferred_restaurant_types) or
                (scorer.name == "cafe" and hasattr(user, 'preferred_cafe_types') and user.preferred_cafe_types)
            )
            initial_k = 50 if has_food_preference else 15
