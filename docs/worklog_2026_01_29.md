...
- 추천 응답 로직 구조화 및 일관성 개선 :
 - 응답 템플릿 도입: 불규칙한 마크다운 형식을 제거하고 번호. [장소명] - 특징 - 추천 이유 형태의 고정 레이아웃을 강제하여 응답 일관성 확보
 - 프롬프트 엔지니어링 최적화: FORMAT_INSTRUCTION 상수를 통해 LLM에 명확한 출력 지침을 전달하고, Few-shot 예시를 추가하여 말투와 형식을 고정
 - 컨텍스트 태깅 방식 변경: 장소 정보를 LLM에 전달할 때 [장소명], [설명] 등 명시적인 태그를 사용하여 데이터 오인식 방지
 - 동적 기준점 삽입: 보정된 위치명(resolved_name)을 시스템 프롬프트와 예시 답변에 직접 삽입하여, 답변 서두에서 기준 위치를 정확히 언급하도록 유도
 - 조건별 지침 강화: 데이트/커플 의도나 카테고리 불분명(mode_unknown) 상황에 따른 특화된 제약 조건을 시스템 메시지에 포함
## 추가 작업
